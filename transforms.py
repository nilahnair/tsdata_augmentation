...
def get_transform(transform):
    if isinstance(transform, str):
        return getattr(sys.modules[__name__], transform)
    elif isinstance(transform, list):
        transforms = []
        for t in transform:
            transform.append(getattr(sys.modules[__name__], transform))
        return # TODO is there a Compose without torchvision?!


def random_curve(window_len: int, sigma=0.05, knot=4):
    """
...
def time_warp(sample: np.ndarray) -> np.ndarray:
    """
    ...
    #
    timesteps = random_curve(window_len, sigma=time_warp_scale)
...
def time_warp_speed(sample: np.ndarray, annotations: np.ndarray, new_window_len: int):
    """
    Computes a time warping that using a random cubic curve
...
# Working
def flipping(sample:np.ndarray):
    return sample[:, :, ::-1] [:,:,::-1,:]

# Working
def magnitude_warping(sample:np.ndarray):
    from scipy.interpolate import CubicSpline
    sigma = 0.2
    knot = 4
...
# Working
def permutation(sample:np.ndarray):
    max_segments=5
    seg_mode="equal"
...
# Working
def time_warping(sample:np.ndarray):
    from scipy.interpolate import CubicSpline
    sigma = 0.2
    knot = 4
    orig_steps = np.arange(sample.shape[1])
...
# Working
def window_warping(sample:np.ndarray):
...
#working
def spawner(sample:np.ndarray, labels, sigma=0.05, verbose=0):
