...
class Network_User(object):
    ...
    def test(self, ea_itera):
        ...
        if self.config['network'] == 'cnn' or self.config['network'] == 'cnn_imu' or self.config['network']=="cnn_transformer":
            ...
            #Loading the model
            network_obj.load_state_dict(torch.load('/data/nnair/icpr2024/networks/cnn_sisfall_act.pt')['state_dict'])
            network_obj.eval()
        ...
        with open('/data/nnair/icpr2024/augment_test/scale_cnn_sisfall.csv', 'a') as myfile:
            for aug in p:
