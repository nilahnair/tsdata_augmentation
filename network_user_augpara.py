...
class Network_User(object):
    ...
    def train(self, ea_itera):
        ...
        torch.save({'state_dict': network_obj.state_dict(),'network_config': network_config}, '/data/nnair/icpr2024/networks/cnn_sisfall_act.pt')
    ...
    def test(self, ea_itera):
        ...
        if self.config['network'] == 'cnn' or self.config['network'] == 'cnn_imu' or self.config['network']=="cnn_transformer":
            ...
            #Loading the model
            network_obj.load_state_dict(torch.load('/data/nnair/icpr2024/networks/cnntrans_motionsense_act.pt')['state_dict'])
            network_obj.eval()
            ...
            #Loading the model
            network_obj.load_state_dict(torch.load('/data/nnair/icpr2024/networks/lstm_motionsense_act.pt')['state_dict'])
            network_obj.eval()
        ...
        with open('/data/nnair/icpr2024/augment_test/scale_cnntrans_motionsense.csv', 'a') as myfile:
            for aug in p:
                ...
                with torch.no_grad():
                    for v, harwindow_batched_test in enumerate(dataLoader_test):
                        ...
                        factor = np.random.normal(loc=1., scale=aug, size=(test_batch_v.shape[0],test_batch_v.shape[2],test_batch_v.shape[3])) #TODO: check if indices are the right ones
